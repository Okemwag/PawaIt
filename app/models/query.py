...
class QueryRequest(BaseModel):
    ...
    query: str = Field(..., description="The question or query from the user", min_length=1)
    user_id: Optional[str] = Field(None, description="Optional user identifier for tracking conversations")
...
class QueryResponse(BaseModel):
    ...
    tokens_used: Optional[int] = Field(None, description="Number of tokens used for this query")
...
class QueryHistory(BaseModel):
    ...
    queries: List[QueryResponse] = Field(default_factory=list, description="List of previous queries and responses")